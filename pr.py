...
def get_kp_forecast_report(days_ahead: int = 0) -> Optional[str]:
    """
    Получает и форматирует отчёт по Kp-индексу за указанный день.

    days_ahead: 0 — сегодня, 1 — завтра, 2 — послезавтра
    Возвращает строку в формате Markdown для Telegram или None.
    """
...
if __name__ == "__main__":
    # Сегодня
    report_today = get_kp_forecast_report(days_ahead=0)
    print(report_today or "Не удалось получить данные за сегодня.")

    print("\n" + "=" * 50 + "\n")

    # Завтра
    report_tomorrow = get_kp_forecast_report(days_ahead=1)
    print(report_tomorrow or "Прогноз на завтра ещё не опубликован.")